...
final class EventSeeder extends Seeder
{
    ...
    {
        $user = User::whereEmail('staff@example.com')->firstOrFail();
        $admin = User::role(Role::Admin)->firstOrFail();

        Model::withoutEvents(function () use ($user, $admin): void {
            foreach ($this->getPending() as $data) {
            ...
            foreach ($this->getApproved() as $data) {
                $event = Event::create($data);
            ...
            foreach ($this->getAvailable() as $data) {
                Event::create($data);
            }

            foreach ($this->getEnded() as $data) {
                $event = Event::create($data);
     ...
     */
    private function getPending(): iterable
    {
        yield [
     ...
     * @return iterable<array<string, mixed>>
     */
    private function getApproved(): iterable
    {
        yield [
     ...
     * @return iterable<array<string, mixed>>
     */
    private function getAvailable(): iterable
    {
        yield [
     ...
     * @return iterable<array<string, mixed>>
     */
    private function getEnded(): iterable
    {
        yield [
            'name' => 'Vzdělávací program pro gymnázium',
            'description' => 'Speciální interaktivní program zaměřený na fyzikální vlastnosti bublin a povrchové napětí pro studenty gymnázia.',
            'start_at' => '2025-01-18 10:00:00',
            'location' => 'Gymnázium Jana Keplera, Praha',
            'contact_person' => 'RNDr. Martin Kučera',
            'contact_email' => 'kucera@gjk.cz',
            'contact_phone' => '+420 233 352 941',
            'is_multi_person' => false,
            'estimated_children_count' => 30,
            'max_workers' => 1,
            'price' => 5500,
            'reward' => 2100,
            'note' => 'Program pro dvě třídy třetího ročníku. Důraz na odborný výklad fyzikálních principů na úrovni středoškolské fyziky.',
            'status' => EventStatus::Completed,
        ];

        yield [
            'name' => 'Podzimní veletrh hraček - prezentační stánek',
            'description' => 'Provoz prezentačního stánku s ukázkami bublinových aktivit na veletrhu dětských hraček a her.',
            'start_at' => '2025-02-15 09:00:00',
            'location' => 'BVV Výstaviště, Brno',
            'contact_person' => 'Ing. Marcela Hrubá',
            'contact_email' => 'marcela.hruba@bvv.cz',
            'contact_phone' => '+420 541 159 753',
            'is_multi_person' => true,
            'estimated_children_count' => 250,
            'max_workers' => 1,
            'price' => 20000,
            'reward' => 3800,
            'note' => 'Dvoudenní akce, stánek 3x4m s elektřinou a přívodem vody. Potřeba vlastních roll-upů a propagačních materiálů.',
            'status' => EventStatus::Completed,
        ];

        yield [
            'name' => 'Velký letní tábor - tematický den',
            'description' => 'Celodenní program s bublinovými aktivitami pro účastníky letního tábora, včetně večerního vystoupení.',
            'start_at' => '2025-03-30 09:30:00',
            'location' => 'Tábor Slunečnice, Křivoklátsko',
            'contact_person' => 'Bc. Ondřej Malý',
            'contact_email' => 'vedouci@taborslunecnice.cz',
            'contact_phone' => '+420 721 987 654',
            'is_multi_person' => true,
            'estimated_children_count' => 85,
            'max_workers' => 1,
            'price' => 13500,
            'reward' => 3000,
            'note' => 'Celodenní program zahrnuje dopolední workshop, odpolední soutěže a večerní vystoupení. Ubytování a strava zajištěna. Lokalita je vzdálená od měst, počítat s delší cestou.',
            'status' => EventStatus::Completed,
        ];
    }
}
